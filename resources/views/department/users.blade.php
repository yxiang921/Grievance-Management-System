...
@section('content')
    <div class="w-full h-auto mt-4 border border-gray-100 shadow-sm rounded-md p-4 bg-white">
        <div class="w-full flex flex-row justify-between items-center pr-2">
            ...
            <a class="primary-btn" href="{{ route('department.user.add') }}">Add new user</a>
        </div>
        <div class="w-full">
            <div class="w-full">
                    ...
                    <tbody>
                        @foreach ($users as $user)
                            <tr class="h-10 hover:bg-primary-100/5">
                                ...
                                <td>
                                    <a href="{{ route('department.user.edit', ['user_id' => $user->id]) }}"
                                        class="underline">Edit</a>
                                    <span class="px-2">|</span>
                                    <a href="{{ route('department.user.delete', ['user_id' => $user->id]) }}" class="underline">Delete</a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
